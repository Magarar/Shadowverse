...
namespace Unit
{
    ...
    public class WebRequest
    {
        ...
        {
            UnityWebRequest request = new UnityWebRequest(url, MethodGet);
        ...
        public static UnityWebRequest Create(string url, string method, string jsonData, string token)
        {
            ...
            request.downloadHandler = new DownloadHandlerBuffer();
            request.timeout = Timeout;
            if (method != null&&!string.IsNullOrEmpty(jsonData))
            {
                UploadHandler uploadHandler = new UploadHandlerRaw(Encoding.UTF8.GetBytes(jsonData));
        ...
        public static UnityWebRequest CreateRaw(string url, string method, string contentType, byte[] data,
            string token)
        {
            UnityWebRequest request = new UnityWebRequest(url, method);
            ...
            if (method != null && !string.IsNullOrEmpty(contentType))
            {
        ...
        public static UnityWebRequest CreateImageUploadForm(string url, string path, byte[] data, string token)
        {
            ...
            {
                new MultipartFormDataSection("path", path, "text"),
                new MultipartFormFileSection("data", data, "file.png", "image/png")
            };
